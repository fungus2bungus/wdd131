...
function updateNotesField() {
  ...
  // TODO Show the travel notes on the form if they are choosing many campuses and require it
    if (value == "many") {
...
form.addEventListener("submit", function (event) {
  ...
  // Let the user know to select at least one campus
    if (selectedCampuses.length === 0) {
        //campusBoxes.setAttribute('required', '');       
        output.textContent = "Please select at least one campus.";
        return;
    }

  
  // TODO Let the user know if they choose many campuses but didn't put a note that they need to add a note
    if (type === "many" && note.trim().length === 0) {
        output.textContent = "Please add a travel note.";
        return;
    }

  
  // TODO Let the user know if they choose many campus but only had one campus selected that they need to choose at least two campuses
